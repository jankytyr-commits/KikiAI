...
def create_pdf():
    ...
    try:
        ...
        print(f"Chyba při načítání JSON: {e}. Spusťte nejdříve generator_dat.py!")
        return

    # 3. Příprava dokumentu s 12 šablonami (pro každý měsíc)
    ...
    # Vytvoření šablon Month_01 až Month_12
    templates = []
    for i in range(1, 13):
    ...
    elements = []
    print("Sázím finální PDF...")

    for week in weeks:
        # Zjištění měsíce pro výběr pozadí
        try:
        ...
        header_row = [
            ...
            Paragraph("Měsíc", header_style), Paragraph("Zahrada", header_style),
            Paragraph("Čína", header_style), Paragraph("Poznámka", header_style)
        ]
        ...
        for day in week["days"]:
            ...
            m_icon = "moon_nov.png"
            if "Úplněk" in m_phase: m_icon = "moon_uplnek.png"
            elif "Dorůstá" in m_phase: m_icon = "moon_dorusta.png"
            elif "Couvá" in m_phase: m_icon = "moon_couva.png"
            moon_html = f"<b>{m_sign}</b> {get_icon_tag(m_icon)} {m_phase}"
            
            # Zahrada (ikona + text)
            g_type = day['moon']['garden_type']
            g_icon = ""
            if "Plod" in g_type: g_icon = "garden_plod.png"
            elif "Kořen" in g_type: g_icon = "garden_koren.png"
            elif "List" in g_type: g_icon = "garden_list.png"
            elif "Květ" in g_type: g_icon = "garden_kvet.png"
            garden_html = f"{get_icon_tag(g_icon)} <b>{g_type}</b>"
            ...
            if n_type == "holiday": note_style = f"<font color='#c0392b'><b>{n_text}</b></font>" # Červená
            elif n_type == "special": note_style = f"<font color='#2980b9'><b>{n_text}</b></font>" # Modrá
            elif n_type == "astro": note_style = f"<font color='grey'><i>{n_text}</i></font>" # Šedá
        ...
        # Tabulka má bílé pozadí (50% opacity v ReportLabu nejde snadno, 
        # tak dáváme plnou bílou pro čitelnost).
