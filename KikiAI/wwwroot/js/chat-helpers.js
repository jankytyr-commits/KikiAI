...
async function sendMessage() {
    ...
    try {
        ...
        debugger; // 3. Response received
        const data = await resp.json();

        if (data.success) {
            debugger; // 4. Success block
            ...
            debugger; // 5. Before addMessage
            try {
                addMessage(botMsg, 'assistant', botTokens, data.metadata);
            } catch (uiError) {
                debugger; // 6. UI Error catch
                console.error('❌ UI Update Failed:', uiError);
                if (chatDiv) {
                    chatDiv.innerHTML += `<div class="msg bot" style="background:#ffdddd;color:black"><strong>FALLBACK RENDER:</strong><br>${botMsg}</div>`;
                }
            }

            updateAllTokenDisplays();
        } else {
            debugger; // 7. API Error
...
function addMessage(text, role, tokens = 0, metadata = null) {
    ...
    // ... (simplified header content for brevity in replacement, assuming existing logic is fine) ...
    // Note: I'm keeping the structure but relying on the existing logic for innerHTML content to avoid huge replacement block if possible, 
    // but since I'm replacing the whole function block, I need to include it.
    ...
    console.log('✅ Message appended to chatDiv');
    chatDiv.scrollTop = chatDiv.scrollHeight;
}
