  ...
  `;
    ...
    if (!cur) {
        ...
        if (!('showDirectoryPicker' in window) || !window.isSecureContext) {
            ...
            fallbackBtn.textContent = 'Vybrat adresář (Fallback)';
            fallbackBtn.style.marginLeft = '8px';
            fallbackBtn.onclick = () => {
            ...
            es.appendChild(fallbackBtn);

            // Update main button to warn
            btn.onclick = () => alert('File System Access API vyžaduje HTTPS. Použijte Fallback tlačítko pro čtení.');
        }
